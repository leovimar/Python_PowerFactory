...
#==============================================================================
def shc_gen_settings(shc, obj, Rf, Xf):
...
#==============================================================================     
# def shc_method_definition(shc, shc_method):
...
#==============================================================================                 
def shc_vsc_model_definition(vscs, shc_method, vsc_model):
    for vsc in vscs:
        if (shc_method == 0) or (shc_method == 1): # VDE 0102 or IEC 60909
            if (vsc_model == 0):   # VDE/IEC: no short-circuit contribution
                ...
                vsc.iNoShcContr = 0
                vsc.iconfed     = 1
                return 'VDE/IEC', 'Static conv. fed drive'
            ...
            if (vsc_model == 0):   # Complete: equivalent SM
                ...
                vsc.iShcModel = 1
                return 'Complete', 'Dyn. voltage support'
...
def shc_data(method, model, obj, ip, Ikss, Iks, Ik, method_col, model_col, obj_col, ip_col, Ikss_col, Iks_col, Ik_col):
    method_col.append(method)
    model_col.append(model)
    obj_col.append(getattr(obj, 'loc_name')) # Object name
    ip_col.append(ip)                        # (kA) Peak short-circuit current
